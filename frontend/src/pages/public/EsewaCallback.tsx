...
export default function EsewaCallbackPage() {
  ...
  const dataPayload = searchParams.get('data');

  useEffect(() => {
    if (!dataPayload) {
      setError('No payment data payload received from eSewa redirect.');
      setLoading(false);
      return;
    }
    ...
    async function verify() {
      try {
        const res = await paymentService.verifyEsewaPayment(dataPayload!);
        if (isMounted) {
          setPaymentData(res);
          setLoading(false);
        }
      ...
      <div className="rounded-xl border border-slate-100 bg-paper-0 shadow-sm p-8 text-center">
        {loading ? (
          <div className="py-12 flex flex-col items-center gap-4">
            ...
            <h2 className="font-display text-xl text-slate-900">Verifying eSewa Payment</h2>
          ...
          <div className="py-6 flex flex-col items-center">
            ...
            <p className="text-sm text-slate-500 mb-6">
              Your appointment payment via eSewa was verified successfully.
            </p>
