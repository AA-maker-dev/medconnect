...
export default function DoctorNotificationsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => doctorDashboardService.fetchNotifications(1, 30),
  });

  const markReadMutation = useMutation({
    mutationFn: (id: string) => doctorDashboardService.markNotificationRead(id),
    onSuccess: () => {
  ...
  const markAllReadMutation = useMutation({
    mutationFn: doctorDashboardService.markAllNotificationsRead,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['doctor', 'notifications'] });
      queryClient.invalidateQueries({ queryKey: ['doctor', 'dashboard-summary'] });
    },
