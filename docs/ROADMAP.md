...
- [x] `GET /api/diseases` — every disease with its specialization attached, powering the "select a disease" step
- [x] `GET /api/appointments/recommended-doctors?diseaseId=...` — the Smart Doctor Recommendation engine. Scores verified, active doctors in the matching specialization on: rating (dampened by review volume, so a 5.0 from 2 reviews doesn't outrank a 4.8 from 200), experience (diminishing returns past ~15 years), completion rate (computed live from appointment history, not a stale stored field), whether they have any active weekly availability at all, and a boost if the requesting patient has seen that doctor before. Returns a `matchScore` and human-readable `matchReasons` per doctor (e.g. "Highly rated", "You've seen this doctor before") for UI transparency.
- [x] Endpoint is public but personalizes when a patient happens to be logged in — added a new `optionalAuthenticate` middleware (populates `req.user` if a valid token is present, never blocks the request otherwise) rather than forcing a login wall just to browse recommendations
- [x] `GET /api/appointments/doctors/:doctorId/slots?date=YYYY-MM-DD` — real slot generation from `DoctorAvailability` weekly schedule, minus already-booked appointments, minus already-passed times if the date is today
- [x] `POST /api/appointments` — booking (patient-only), re-validates slot availability at submission time to close most of the race-condition window, notifies the doctor, returns a confirmation-ready payload
- [x] `GET /api/appointments/:id` — confirmation/detail lookup, restricted to the patient or doctor who own the appointment (or admin)
- [x] Frontend: real `/doctors` directory (search + specialization filter + sort), `/doctors/:id` detail page with an embedded 14-day date picker + live slot grid + booking form, `/search` disease-picker with scored/reasoned doctor recommendations, and a booking confirmation page
- [x] Seed script: availability now seeded for all 6 demo doctors (previously just 2), so the booking flow has real bookable slots across the whole demo doctor roster

## Phase 8 deliverables checklist

- [x] Every messaging rule from the spec enforced at the data-model level, not just in application logic: a `ChatRoom` exists 1:1 with an `Appointment`, so "can this user access this room" collapses to "is this user the patient or doctor on the appointment this room belongs to" — a patient literally cannot message another patient, a doctor cannot message another doctor, and access is checked on every REST call and every socket event, not just once at connection time
- [x] Real-time text messaging over Socket.io (`message:send` → persisted → broadcast to both participants), with a JWT handshake reusing the same access token as REST (from Phase 2)
- [x] Typing indicators (`typing:start`/`typing:stop`, auto-clears after 2.5s of inactivity or on blur/disconnect)
- [x] Delivered status (flips the moment the recipient's socket joins the room) and seen status (flips when the room is actively open/visible), each with its own DB timestamp and its own broadcast event
- [x] Online/offline presence, tracked per-user (not per-socket, so multiple open tabs don't falsely show "offline" when one closes) and broadcast on every state change, plus a bulk `presence:check` event so a room list can show every contact's status at once without N round trips
- [x] Unread counter per room (computed server-side from message status, not trusted from the client) and a real-time date-separator message list
- [x] Message search within a room (backend `contains`/insensitive query, not just client-side filtering)
- [x] Image and PDF sharing via a REST upload endpoint (local disk storage in dev — architecture-ready for a Cloudinary swap later, same pattern as the SMS provider stub) — the file lands on disk first, then a normal `message:send` socket event carries the URL, so attachments appear in real time exactly like text does
- [x] Reply-to-message (quoted preview, works on your own or the other party's messages) and delete-own-message (soft delete, preserves the row for the other party's "message was deleted" state rather than breaking reply references)
- [x] Auto-scroll to bottom on new messages, "load earlier messages" pagination, animated message bubbles
- [x] Emoji picker — a small curated grid built in-house rather than pulling in a heavy emoji-picker dependency not already in the stack
- [x] Doctor dashboard's Phase 5 Messages placeholder replaced with the real chat UI; Patient dashboard gained a new Messages page + nav item (the original spec's Doctor Dashboard card list mentioned Messages explicitly, but messaging is inherently bidirectional — patients need it too)
- [x] Seed script extended: chat rooms are now created for every seeded APPROVED/COMPLETED appointment (previously chat rooms only ever got created by the live approval API, so seeded appointments had none), plus one realistic demo conversation with a reply reference so the UI has real content to show immediately

### A note on how this phase was built

Partway through this phase, the sandbox session this was being built in reset unexpectedly, and the accumulated Phases 1–7 codebase in the working directory was lost (though nothing delivered to you was affected, since you'd already applied each phase's update zip to your own project). Continuing required re-uploading your current project so the Phase 8 work could be verified against your actual Phase 1–7 baseline rather than a remembered/reconstructed one — the same import-resolution, export-matching, and brace-balance checks used throughout this build were re-run against the re-uploaded project before any Phase 8 code was written, and again after, to make sure nothing drifted.

## Notes for future phases

- Presence is currently broadcast to every connected socket, not scoped to shared chat rooms — a deliberate simplicity tradeoff at this app's scale (a clinic's patient roster, not a social network). Worth revisiting if this ever needs to scale past that.
- Chat file uploads use local disk storage (`backend/uploads/chat`, served statically) rather than Cloudinary, even though `CLOUDINARY_*` env vars already exist from Phase 1 — swapping is a same-shape change contained entirely to `middleware/chatUpload.ts`; nothing else references the disk path directly, only the `fileUrl` the endpoint returns.
- Voice notes are schema-ready (`MessageType.VOICE_NOTE` exists) but not implemented — the spec listed this as explicitly optional, and it needs browser audio recording (`MediaRecorder`) on the frontend that the other message types don't, so it's a reasonable place to have drawn the line for this phase.
- The chat REST upload endpoint and the socket `message:send` event are two separate round trips for attachments (upload the file, then send a message event with its URL) rather than one atomic operation — if the socket call fails after a successful upload, the file exists on disk with no message referencing it. Low-stakes (an orphaned file, not corrupted data) but worth a cleanup job eventually.
